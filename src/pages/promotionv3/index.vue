    ...
    <el-drawer v-model="detailDrawer" title="活动详情" size="35%" direction="rtl">
      ...
      <vxe-table :data="promotionDetail.rules">
        <vxe-column title="类型" width="100">
          <template #default="data">
            <el-text>{{ ruleTypes.find((type: any) => Number(type.value) === data.row.contents?.discountStrategy?.type)?.name || "其他" }}</el-text>
          </template>
        </vxe-column>
        ...
        <vxe-column title="折扣值" width="80">
          <template #default="data">
            <el-text>{{ data.row.contents?.discountStrategy?.value || "-" }}</el-text>
          </template>
        </vxe-column>
